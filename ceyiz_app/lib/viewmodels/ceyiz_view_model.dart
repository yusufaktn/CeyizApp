...
class CeyizViewModel extends BaseItemViewModel {
  final CeyizService _ceyizService;

  CeyizViewModel({
    required CeyizService ceyizService,
    required PhotoService photoService,
  })  : _ceyizService = ceyizService,
  ...
  Future<void> loadItems() async {
    setLoading(true);
    try {
      final items = await _ceyizService.getItems();
  ...
  Future<void> loadCategories() async {
    try {
      final categories = await _ceyizService.getCategories();
  ...
  Future<void> addItem(
    ...
    try {
      ...
      print('CeyizViewModel.addItem: Creating new item with id: ${item.id}');
      await _ceyizService.addItem(item);
      print('CeyizViewModel.addItem: Item saved to storage');

      // Doğrudan listeye ekle ve bildir
      items.add(item);
      clearTempPhotos();
      setLoading(false);
      print(
          'CeyizViewModel.addItem: Notifying listeners with updated list (${items.length} items)');
      notifyListeners();
    } catch (e) {
      print('CeyizViewModel.addItem: ERROR - $e');
  ...
  Future<void> updateItem(dynamic item) async {
    if (item is! CeyizItemModel) {
      throw ArgumentError('Item must be a CeyizItemModel');
    }

    setLoading(true);
    try {
      await _ceyizService.updateItem(item);
  ...
  Future<void> deleteItem(String id) async {
    setLoading(true);
    try {
      await _ceyizService.deleteItem(id);
  ...
  Future<void> togglePurchaseStatus(String id) async {
    try {
      ...
      if (index != -1) {
        ...
        await _ceyizService.updateItem(updatedItem);
  ...
  Future<void> addCategory(String category) async {
    await _ceyizService.addCategory(category);
    await loadCategories();
    notifyListeners();
  }

  Future<void> removeCategory(String category) async {
    await _ceyizService.removeCategory(category);
    await loadCategories();
    notifyListeners();
  }
}
